...
Applications use a JNDI InitialContext, itself obtained from an InitialContextFactory, to look up JMS objects such as ConnectionFactory. The Qpid JMS client provides an implementation of the InitialContextFactory in class *com.rabbitmq.client.jms.jndi.JmsInitialContextFactory*. This may be configured and used in three main ways:
...
1.  Via jndi.properties file on the Java Classpath.
    ...
    to value *com.rabbitmq.client.jms.jndi.JmsInitialContextFactory*, the Qpid InitialContextFactory implementation will
...
2.  Via system properties.
    ...
    By setting the *java.naming.factory.initial* system property to value *com.rabbitmq.client.jms.jndi.JmsInitialContextFactory*,
...
3.  Programmatically using an environment Hashtable.
    ...
    The InitialContext may also be configured directly by passing an environment during creation:
        ...
        env.put(Context.INITIAL_CONTEXT_FACTORY, "com.rabbitmq.client.jms.jndi.JmsInitialContextFactory");
...
The client uses Logger names residing within the *com.rabbitmq.client.jms* hierarchy, which you can use to configure a logging implementation based on your needs.
...
+ Add the option *amqp.traceFrames=true* to your connection URI to have the client add a protocol tracer to Proton, and configure the *com.rabbitmq.client.jms.provider.amqp.FRAMES* Logger to *TRACE* level to include the output in your logs.
...
2.  Set a JmsTracer instance to use directly on the JmsConnectionFactory object.
        ...
        io.opentracing.Tracer tracer = ...;
        com.rabbitmq.client.jms.tracing.JmsTracer jmsTracer =
                com.rabbitmq.client.jms.tracing.opentracing.OpenTracingTracerFactory.create(tracer);
